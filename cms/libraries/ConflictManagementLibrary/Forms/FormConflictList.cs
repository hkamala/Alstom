...

{
    public partial class FormConflictList : Form
    {
        ...
        private const string myName = "Trip Conflict List";
        ...
        private List<FormReservation> myFormReservations = new List<FormReservation>();
        ...
        private void mnuShowRoutePlan_Click(object sender, EventArgs e)
        {
            try
            {
                var lvItem = lvTrips.SelectedItems[0];
                if (lvItem != null)
                {
                    var form = new Forms.FormRoutePlan();
                    var trip = FindTrip(lvItem.SubItems[1].Text, lvItem.SubItems[3].Text);
                    if (trip == null) return;
                    form.AssociateTrip(trip);
                    form.Show(this);
    ...
    public class ItemComparer : IComparer
    {
        ...
        public int CompareDate(object x, object y)
        {
            ...
            try
            {
                ...
                System.DateTime firstDate = DateTime.ParseExact(((ListViewItem)x).SubItems[col].Text, format, CultureInfo.InvariantCulture);
                System.DateTime secondDate = DateTime.ParseExact(((ListViewItem)y).SubItems[col].Text, format, CultureInfo.InvariantCulture);

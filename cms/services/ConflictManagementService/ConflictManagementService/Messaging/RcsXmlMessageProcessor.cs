...
abstract class RcsXmlMessageProcessor : MessageProcessor
{
    ...
    override protected void ProcessMessage(IMessage msg)
    {
        if (msg != null && Connection != null)
        {
            ...
            {
                Connection.DispatchMessageToSubscribers(new Subscription(new Channel(type, channel), msgType), hdr, dataMsgElement, msgProperties, msg);
            }
        }
    }
    ...
    private bool DeserializeMsg(IMessage msg, out Dictionary<string, string> hdr, out ChannelType type, out string channel, out string msgType, out XElement? dataMsgElement, out Dictionary<string, string> msgProperties)
    {
        ...
        try
        {
            ...
            if (msg is ITextMessage)
                xml = ((ITextMessage)msg).Text;
            else if (msg is IBytesMessage)
                xml = Encoding.ASCII.GetString(((IBytesMessage)msg).Content);
            else
    ...
    public override IMessage? CreateMessage(Dictionary<string, string> hdr, XElement msg, Dictionary<string, string> msgProperties, List<string>? namespaces = null)
    {
        try
        {
	...
	public override IMessage? CreateMessage(string body, IDictionary<string, object> properties)
	{
        if (Connection == null || !(Connection is AMQPConnection amqpConn))
            return null;

        ISession? session = amqpConn.Session;
        if (session == null)
            return null;

        IMessage msg = session.CreateTextMessage(body);
        AddPropertiesToMessage(msg, properties);

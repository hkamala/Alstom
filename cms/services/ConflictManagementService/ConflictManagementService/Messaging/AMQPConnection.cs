    ...
    public override void DispatchMessageToSubscribers(Subscription subscription, Dictionary<string, string> hdr, XElement? msg, Dictionary<string, string> msgProperties, IMessage rawMsg)
    {
        if (msg != null)
        {
            ...
            if (isSubscribed)
            {
                ...
                Log.Error($"{subscription} is not subscribed. Nothing to worry about, ConflictManagementService does not need information in this message");
        }
    }
            ...
            if (message != null)
            {
                lock (this)
                {
                    ...
                    if (AllowExtensiveMessageLogging)
                        Log.Debug($"Sent message, channel {channel}: {message}");
                }
            }
            else
            {
